...
export default class NoBareStrings extends Rule {
  ...
  _formatBareString(bareStringText) {
    return bareStringText
      .replace(/\s{2,}/g, ' ')
      .replace('\n', ' ')
      .trim();
  }

  _formatKey(simplifiedPath, bareStringTextFormatted) {
    const truncatedString = _truncate(_kebabCase(bareStringTextFormatted), {
      length: 50,
      separator: '-',
      omission: '',
    });
    return `${simplifiedPath}__${truncatedString}`;
  }
  ...
  _checkNodeAndLog(node, additionalDescription, loc) {
    ...
    try {
      fs.readFileSync('./bare-strings.yaml', 'utf8');
    } catch {
      fs.writeFileSync('./bare-strings.yaml', '');
    }

    const bareStringsFile = fs.readFileSync('./bare-strings.yaml', 'utf8');
    let bareStringsObject = parse(bareStringsFile);
    ...
    switch (node.type) {
      case 'TextNode': {
        ...
        if (bareStringText) {
          const simplifiedPath = this._simplifyPath(this.filePath);
          const bareStringTextFormatted = this._formatBareString(bareStringText);
          const key = this._formatKey(simplifiedPath, bareStringTextFormatted);
